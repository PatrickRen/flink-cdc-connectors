...
/**
 * The {@link RecordEmitter} implementation for {@link MongoDBSourceReader}.
 *
    ...
    private void processElement(
        ...
        if (isHeartbeatEvent(element)) {
            ...
            if (splitState.isStreamSplitState()) {
                ...
                Map<String, String> startupOffset = new HashMap<>();
                startupOffset.put(RESUME_TOKEN_FIELD, getOffsetValue(element, RESUME_TOKEN_FIELD));
                startupOffset.put(TIMESTAMP_FIELD, getOffsetValue(element, TIMESTAMP_FIELD));
                splitState
                        .asSnapshotSplitState()
                        .setHighWatermark(new ChangeStreamOffset(startupOffset));
            }
