...
public class MongoDBSourceOptions {
    ...
    @Experimental
    public static final ConfigOption<Boolean> SCAN_PARALLELISM_SNAPSHOT_ENABLED =
            ConfigOptions.key("scan.parallelism.snapshot.enabled")
                    .booleanType()
                    .defaultValue(false)
                    .withDescription("Whether enable parallelism snapshot. Defaults to false.");

    @Experimental
    public static final ConfigOption<Integer> SCAN_PARALLELISM_SNAPSHOT_CHUNK_SIZE_MB =
            ConfigOptions.key("scan.parallelism.snapshot.chunk.size.mb")
                    .intType()
                    .defaultValue(64)
                    .withDescription(
                            "The chunk size mb of parallelism snapshot. Defaults to 64mb.");
}
