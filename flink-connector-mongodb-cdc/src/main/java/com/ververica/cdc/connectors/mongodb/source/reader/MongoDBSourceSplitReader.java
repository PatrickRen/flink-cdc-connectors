...
public class MongoDBSourceSplitReader implements SplitReader<SourceRecords, SourceSplitBase> {
    ...
    @Nullable private Fetcher<SourceRecords, SourceSplitBase> currenFetcher;
    ...
    protected void checkSplitOrStartNext() throws IOException {
        ...
        if (canAssignNextSplit()) {
            SourceSplitBase nextSplit = splits.poll();
            if (nextSplit == null) {
            ...
            if (nextSplit.isSnapshotSplit()) {
                if (currenFetcher == null) {
                    final MongoDBFetchTaskContext taskContext =
                            dialect.createFetchTaskContext(nextSplit, sourceConfig);
