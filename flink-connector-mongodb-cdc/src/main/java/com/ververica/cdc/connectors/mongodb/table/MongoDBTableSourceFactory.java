...
public class MongoDBTableSourceFactory implements DynamicTableSourceFactory {
    ...
    public DynamicTableSource createDynamicTableSource(Context context) {
        ...
        boolean enableParallelRead = config.get(SCAN_PARALLELISM_SNAPSHOT_ENABLED);

        int chunkSizeMB = config.get(SCAN_PARALLELISM_SNAPSHOT_CHUNK_SIZE_MB);
    ...
    public Set<ConfigOption<?>> optionalOptions() {
        ...
        options.add(SCAN_PARALLELISM_SNAPSHOT_ENABLED);
        options.add(SCAN_PARALLELISM_SNAPSHOT_CHUNK_SIZE_MB);
        return options;
    }
}
