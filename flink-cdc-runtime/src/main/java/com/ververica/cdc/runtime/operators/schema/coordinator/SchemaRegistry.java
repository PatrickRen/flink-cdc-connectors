...
public class SchemaRegistry implements OperatorCoordinator, CoordinationRequestHandler {
    ...
    public CompletableFuture<CoordinationResponse> handleCoordinationRequest(
        ...
        } else if (request instanceof SchemaRequest) {
            return CompletableFuture.completedFuture(
                    handleSchemaRequest(((SchemaRequest) request)));
        } else {
    ...
    private SchemaResponse handleSchemaRequest(SchemaRequest schemaRequest) {
        LOG.info("Handling schema request: {}", schemaRequest);
        int schemaVersion = schemaRequest.getSchemaVersion();
        TableId tableId = schemaRequest.getTableId();
        if (schemaVersion == SchemaRequest.LATEST_SCHEMA_VERSION) {
            return new SchemaResponse(schemaManager.getLatestSchema(tableId).orElse(null));
        } else {
            try {
                return new SchemaResponse(schemaManager.getSchema(tableId, schemaVersion));
            } catch (IllegalArgumentException iae) {
                LOG.warn(
                        "Some client is requesting an non-existed schema for table {} with version {}",
                        tableId,
                        schemaVersion);
                return new SchemaResponse(null);
            }
        }
    }
}
